...
class WhooshSearchBackend(BaseSearchBackend):
    ...
    def __init__(self, connection_alias, **connection_options):
        ...
        if self.use_file_storage and not self.path:
            raise ImproperlyConfigured(
                f"You must specify a 'PATH' in your settings for connection '{connection_alias}'."
            )
    ...
    def setup(self):
        ...
        if self.use_file_storage and not os.access(self.path, os.W_OK):
            raise IOError(
                f"The path to your Whoosh index '{self.path}' is not writable for the current user/group."
            )
    ...
    def remove(self, obj_or_string, commit=True):
        ...
        try:
            self.index.delete_by_query(q=self.parser.parse(f'{ID}:"{whoosh_id}"'))
        except Exception as e:
            if not self.silently_fail:
                raise
...
class WhooshSearchQuery(BaseSearchQuery):
    ...
    def build_query_fragment(self, field, filter_type, value):
        ...
        index_fieldname = (
            ''
            if field == 'content'
            else f'{connections[self._using].get_unified_index().get_index_fieldname(field)}:'
        )
